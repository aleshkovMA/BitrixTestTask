		...
		case "orm":
			...
			foreach ($arFields as $columnName => $columnInfo)
			{
				...
				switch ($type)
				{
					...
					if ($columnInfo["default"] !== '' && !is_numeric($columnInfo["default"]))
						{
							$defaultValue = strtolower($columnInfo["default"]);
							if (strlen($defaultValue) > 2)
							{
								if (substr_compare($defaultValue, '()', -2, 2, true) === 0)
									$defaultValue = substr($defaultValue, 0, -2);
							}
				...
				if ($objectSettings)
				{
					...
					$fields[$columnName] = substr($fields[$columnName], 0, -1).",\n";
				}
				else
				{
					$fields[$columnName] = "\t\t\t"
			...
			foreach ($arParents as $columnName => $parentInfo)
			{
				...
				{
					$columnNameEx = strtoupper($parentClassName);
				}
			...
			echo "\n\nFile: /bitrix/modules/".$moduleName."/lib/".strtolower($className)."table.php";
			echo "<hr>";
			echo "<pre>";
			echo "&lt;", "?", "php\n";
			...
			echo "}\n";
			echo "</pre>";
			echo "File: /bitrix/modules/".$moduleName."/lang/ru/lib/".strtolower($className)."table.php";
			echo "<hr>";
			echo "<pre>";
			echo "&lt;", "?\n";
