...
else
{
	...
	if($new != "" && strlen($filename) > 0 && $io->FileExists($abs_path))		// if we want to make new file , but the file with the same name alredy exist, let's abuse
	{
		...
		if($strWarning == "")
		{
			...
			else
			{
				...
				if(COption::GetOptionString($module_id, "log_page", "Y")=="Y")
				{
					...
					if ($new != "" && strtolower($new) == 'y' && strlen($filename) > 0)
						CEventLog::Log(
							"content",
							"FILE_ADD",
							"fileman",
...
if (!$ismenu)
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => ($mode == 'text' ? "return; " : "" )."window.location='fileman_file_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($originalPath).($new != ""? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"").(strlen($templateID)>0? "&amp;templateID=".urlencode($templateID):"")."';",
...
if ($ismenu)
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => "window.location='"."fileman_menu_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($arParsedPath["PREV"])."&amp;name=".UrlEncode($regs[1]).($new != ""? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"")."';"
	);
}
else
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => "window.location='"."fileman_html_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($originalPath).($new != "">0? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"").(strlen($template)>0? "&amp;template=".urlencode($template):"").(strlen($templateID)>0? "&amp;templateID=".urlencode($templateID):"")."';"
	);
}
...
<form method="POST" action="<?= $APPLICATION->GetCurPage().'?'.$addUrl."&amp;path=".UrlEncode($path).($new != "">0? "&amp;new=Y":"")?>" name="ffilemanedit">
